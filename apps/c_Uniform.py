...
def app():
    if "Uniform_txt_dict" not in st.session_state:
        proj_root = get_proj_root()
        fil = os.path.join(proj_root, "text_files", "c_Uniform.txt")
        st.session_state["Uniform_txt_dict"] = load_dict_txt(fil)
    if "uniform_dist" not in st.session_state:
        st.session_state["uniform_dist"] = Uniform_distribution(
            key_root="uniform_dist", session_state=st.session_state
        )
    dist = st.session_state["uniform_dist"]

    st.header(st.session_state["Uniform_txt_dict"]["main_title"])

    # * ddb - General information about distribution
    st_expandable_box(st.session_state["Uniform_txt_dict"], "lvl_0_title", "lvl_0_text")
    ...
    # * ddb - General information about distribution
    st_expandable_box(st.session_state["Uniform_txt_dict"], "lvl_1_title", "lvl_1_text")
