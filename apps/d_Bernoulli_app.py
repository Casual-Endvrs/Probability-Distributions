...
def app():
    if "Bernoulli_txt_dict" not in st.session_state:
        proj_root = get_proj_root()
        fil = os.path.join(proj_root, "text_files", "d_Bernoulli.txt")
        st.session_state["Bernoulli_txt_dict"] = load_dict_txt(fil)
    if "bernoulli_dist" not in st.session_state:
        st.session_state["bernoulli_dist"] = Bernoulli_distribution(
            key_root="bernoulli_dist", session_state=st.session_state
        )
    dist = st.session_state["bernoulli_dist"]

    st.header(st.session_state["Bernoulli_txt_dict"]["main_title"])

    # * ddb - General information about distribution
    st_expandable_box(
        st.session_state["Bernoulli_txt_dict"], "lvl_0_title", "lvl_0_text"
    )
    ...
    # * ddb - General information about distribution
    st_expandable_box(
        st.session_state["Bernoulli_txt_dict"], "lvl_1_title", "lvl_1_text"
    )
