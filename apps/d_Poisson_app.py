...
def app():
    if "Poisson_txt_dict" not in st.session_state:
        proj_root = get_proj_root()
        fil = os.path.join(proj_root, "text_files", "d_Poisson.txt")
        st.session_state["Poisson_txt_dict"] = load_dict_txt(fil)
    if "poisson_dist" not in st.session_state:
        st.session_state["poisson_dist"] = Poisson_distribution(
            key_root="poisson_dist", session_state=st.session_state
        )
    dist = st.session_state["poisson_dist"]

    st.header(st.session_state["Poisson_txt_dict"]["main_title"])

    # * ddb - General information about distribution
    st_expandable_box(st.session_state["Poisson_txt_dict"], "lvl_0_title", "lvl_0_text")
    dist.create_sliders()
    ...
    # * ddb - General information about distribution
    st_expandable_box(st.session_state["Poisson_txt_dict"], "lvl_1_title", "lvl_1_text")
