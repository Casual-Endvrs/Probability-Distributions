...
class Uniform_distribution:
    def __init__(
        ...
        self.dist_values = np.array(
            [0.0, 1.0]
        )  # list of the distribution properties - [start value, width]
    ...
    def reset_sim(
        ...
        self.bin_width = self.dist_values[1] / self.sim_num_bins

        dist_start = self.dist_values[0] - self.bin_width
        dist_end = self.dist_values[0] + self.dist_values[1] + self.bin_width

        self.sim_bins_markers = np.linspace(dist_start, dist_end, self.sim_num_bins + 3)
        self.sim_bins_mid = self.sim_bins_markers[:-1] + self.bin_width / 2
        self.sim_bins_cnts = np.zeros(int(self.sim_num_bins) + 2)
    ...
    def _create_dist(
        ...
        self.dist = stats.uniform(self.dist_values[0], self.dist_values[1])
    ...
    def _get_sim_bins_normalized(
        ...
        mean_val = self.sim_total_entries / (len(self.sim_bins_cnts) - 2)

        return self.sim_bins_cnts / mean_val / self.dist_values[1]
    ...
    def _update_plot_rng(
        ...
        self.plot_rng = np.array(
            [self.sim_bins_markers[0] - 0.1, self.sim_bins_markers[-1] + 0.1]
        )
